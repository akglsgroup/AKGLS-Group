        ...
        leadForm.addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (geoRes && !geoRes.error) {
              country = geoRes.country_name || geoRes.country || 'Unknown';
              city = geoRes.city || 'Unknown';
              region = geoRes.region || 'Unknown';
            }
          } catch (_) {}
          ...
          const fullPayload = {
            ...
            email: raw.email || 'no-email@captured.com',
            phone: raw.phone || '',
            websiteUrl: raw.websiteUrl || '',
            companyName: raw.companyName || '',
            primaryGoal: raw.primaryGoal || 'Consultation Request',
            notes: raw.notes || '',
            pageAddress: raw.pageAddress || window.location.href,
            pageTitle: raw.pageTitle || document.title,
            country: country,
            city: city,
            region: region,
            time: nowIso,
          ...
          // 1. Direct Cloud Firestore write (Global Real-Time - works with zero backend on Hostinger / CDN)
          const firestoreFields = {
            fields: {
              id: { stringValue: fullPayload.id },
              name: { stringValue: String(fullPayload.name) },
              email: { stringValue: String(fullPayload.email) },
              phone: { stringValue: String(fullPayload.phone) },
              websiteUrl: { stringValue: String(fullPayload.websiteUrl) },
              companyName: { stringValue: String(fullPayload.companyName) },
              primaryGoal: { stringValue: String(fullPayload.primaryGoal) },
              notes: { stringValue: String(fullPayload.notes) },
              pageAddress: { stringValue: String(fullPayload.pageAddress) },
              pageTitle: { stringValue: String(fullPayload.pageTitle) },
              country: { stringValue: String(fullPayload.country) },
              city: { stringValue: String(fullPayload.city) },
              region: { stringValue: String(fullPayload.region) },
              time: { stringValue: fullPayload.time },
              status: { stringValue: 'New' },
              assignedTo: { stringValue: 'Unassigned' }
            }
          };

          const firestorePromise = fetch('https://firestore.googleapis.com/v1/projects/ask-amrish/databases/(default)/documents/leads?documentId=' + encodeURIComponent(leadId) + '&key=AIzaSyAK7JkxcKSJdMeQhlj-qXE1Va4Y25jcjPw', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(firestoreFields)
          }).catch(err => console.warn('[Direct Firestore Sync Notice]', err));

          // 2. Express Backend API call (in case server is active)
          ...
          // 3. Save to local browser storage backup
          try {
            const localStr = localStorage.getItem('akgls_system_leads') || '[]';
            let localLeads = JSON.parse(localStr);
            if (!Array.isArray(localLeads)) localLeads = [];
            localLeads.unshift(fullPayload);
