...
export default function FinanceMarketingPage({ onBackToHome, openProposalForm }: FinanceMarketingPageProps) {
  ...
  return (
    <>
      ...
      {/* 👑 HERO SECTION */}
      <section className="relative pt-6 pb-16 sm:pt-8 md:pt-10 md:pb-24 text-left bg-[#020408] border-b border-slate-950 overflow-hidden">
        {/* Top ambient highlight bridge */}
        <div className="absolute top-0 inset-x-0 h-24 bg-gradient-to-b from-brand-teal/10 via-emerald-400/5 to-transparent pointer-events-none" />
        {/* Background grid */}
                  ...
                  <div className="space-y-2.5">
                    <div className="flex justify-between items-center text-[10px] font-mono">
                      <span className="text-emerald-400 font-bold flex items-center gap-1.5">
                        ...
                        Fintech & Wealth Trajectory
                      </span>
                      <button 
                        onClick={() => setHeroViewMode('calculator')}
                        className="text-emerald-400 hover:underline text-[9.5px] cursor-pointer flex items-center gap-1"
                      >
                        Adjust Yield Engine →
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-950/80 p-2.5 rounded-xl border border-slate-900 text-left">
                      <div>
                        ...
                        <span className="text-sm font-black font-display text-white mt-0.5 block">{akglsLeads} apps</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-500 font-mono uppercase block">Closed Accounts</span>
                        <span className="text-sm font-black font-display text-emerald-400 mt-0.5 block">{akglsSignedClients} partners</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-500 font-mono uppercase block">Estimated Revenue</span>
                        <span className="text-sm font-black font-display text-brand-teal mt-0.5 block">${(projectedFeesEarned / 1000).toFixed(0)}k/mo</span>
                      </div>
                    </div>
                  </div>
                }
