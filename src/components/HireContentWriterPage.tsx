...
export default function HireContentWriterPage({ onBackToHome, openProposalForm }: HireContentWriterPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#02040d] text-slate-100 selection:bg-emerald-500/30 selection:text-emerald-300 font-sans antialiased overflow-x-hidden">
      ...
      <nav id="writer-hiring-nav" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between border-b border-slate-900/60 sticky top-0 bg-[#02040d]/85 backdrop-blur z-50">
        <button 
      ...
      <AnimatePresence>
        {showStickyCta && (
          <motion.div 
            ...
            className="fixed bottom-0 left-0 right-0 bg-[#04081c]/95 border-t border-emerald-500/30 backdrop-blur-md py-4 z-50 shadow-2xl"
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-ping shrink-0" />
                <p className="text-xs sm:text-sm text-slate-300">
                  Ready to optimize your site copy? Hire a <strong className="text-emerald-400 font-bold">dedicated Content Expert</strong> from AKGLS today.
                </p>
              </div>
              <div className="flex items-center gap-3">
                <a 
                  ...
                  target="_blank"
                  referrerPolicy="no-referrer"
                  className="bg-emerald-950/30 text-emerald-400 border border-emerald-900 px-4 py-2 rounded-lg text-xs font-mono hover:bg-emerald-950/65 transition"
                >
                  WhatsApp Expert
                </a>
                <button 
                  onClick={() => {
                    const form = document.getElementById('free-content-audit-stage');
                    if (form) form.scrollIntoView({ behavior: 'smooth' });
                  }}
                  className="bg-gradient-to-r from-emerald-400 to-cyan-500 text-slate-950 font-bold px-5 py-2 rounded-lg text-xs hover:opacity-95 transition"
                >
                  Get Free Audit Code
                </button>
              </div>
